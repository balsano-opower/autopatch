...
public class JdbcMigrationLauncherTest extends MigrationListenerTestBase {
    ...
    private void runReadOnlyTest(String mode) throws MigrationException{
        ...
        switch (mode) {
            case MIGRATION: testLauncher.doMigrations();
                break;
            case ROLLBACK: testLauncher.doRollbacks(ROLLBACK_LEVELS);
                break;
            default: throw new MigrationException("Unsupported mode: " + mode);
        }
        mockControl.verify();
    }
